...
class StorageVirtualNode:
    def __init__(
        ...
        self.bandwidth = bandwidth * 1_000_000  # Mbps → bps
    ...
    def _generate_chunks(self, file_id: str, file_size: int) -> List[FileChunk]:
        ...
        for i in range(num_chunks):
            ...
            actual_size = min(chunk_size, file_size - i * chunk_size)
            chunks.append(FileChunk(chunk_id=i, size=actual_size, checksum=checksum))
        return chunks

    def initiate_file_transfer(self, file_id: str, file_name: str, file_size: int, source_node: Optional[str] = None):
    ...
    def process_chunks_parallel(self, file_id: str, source_node: str, max_workers: int = 4) -> int:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = {executor.submit(self._simulate_chunk_transfer, c, source_node): c for c in pending_chunks[:max_workers]}
